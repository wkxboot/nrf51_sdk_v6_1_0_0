...
static uint32_t ble_sls_notify_device_info( ble_sls_t *p_sls,uint16_t value_handle,uint8_t *ptr_data,uint16_t data_len)
{
    ...
    if (p_sls->conn_handle != BLE_CONN_HANDLE_INVALID && value_handle!= BLE_CONN_HANDLE_INVALID)
    {
...
static void device_manager_init(void)
{
    ...
    init_data.clear_persistent_data = false;//(nrf_gpio_pin_read(BOND_DELETE_ALL_BUTTON_ID) == 0);
