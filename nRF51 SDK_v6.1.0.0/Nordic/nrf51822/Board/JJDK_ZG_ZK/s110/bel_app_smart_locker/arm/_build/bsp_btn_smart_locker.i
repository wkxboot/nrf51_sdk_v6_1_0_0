...
#line 12 "..\\bsp_btn_smart_locker.c"
...
app_timer_id_t sys_run_led_timer_id;
 ...
 uint8_t uv_lamp_status=2;
 uint8_t fan_negative_ion_status=2;
 uint8_t elec_lock_status=2;
 uint8_t uv_lamp_door_status=2;
...
#line 52 "..\\bsp_btn_smart_locker.c"
...
void bsp_board_buttons_init(void)
{
    ...
    do { uint32_t ERR_CODE = app_button_init((buttons), (sizeof(buttons) / sizeof(buttons[0])), (((uint32_t)((((50) * (uint64_t)32768) + ((((smart_locker_local_prescal) + 1) * 1000) / 2)) / (((smart_locker_local_prescal) + 1) * 1000)))), (0) ? app_button_evt_schedule : 0); do { const uint32_t LOCAL_ERR_CODE = (ERR_CODE); if (LOCAL_ERR_CODE != ((0x0) + 0)) { do { app_error_handler((LOCAL_ERR_CODE), 240, (uint8_t*) "..\\bsp_btn_smart_locker.c"); } while (0); } } while (0); } while (0); 
}
...
uint32_t bsp_smart_locker_board_init(uint32_t prescale)
{
   ...
   do { if (((err_code) != ((0x0) + 0)) && ((err_code) != ((0x0) + 7))) { return err_code; } } while (0);
	 app_timer_start(sys_run_led_timer_id,((uint32_t)((((1000) * (uint64_t)32768) + ((((smart_locker_local_prescal) + 1) * 1000) / 2)) / (((smart_locker_local_prescal) + 1) * 1000))),0);
   return err_code;
   ...
   if( button_action== 1 )
	{
         switch (pin_no)
        {
          ...
          break;						
          default:
          do { app_error_handler((pin_no), 331, (uint8_t*) "..\\bsp_btn_smart_locker.c"); } while (0);
          break;
				}
	}
	else if( button_action == 0 )
	{
        switch (pin_no)
        {
          ...
          break;						
          default:
          do { app_error_handler((pin_no), 353, (uint8_t*) "..\\bsp_btn_smart_locker.c"); } while (0);
          break;
				}
		}
}
